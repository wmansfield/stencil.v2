...
{
    public partial interface IWidgetStore
    {
        
        Task<Widget> GetDocumentAsync(Guid shop_id, Guid widget_id);
        
        Task<bool> CreateDocumentAsync(Widget model);
        Task<bool> DeleteDocumentAsync(Widget model);
        
        
        Task<ListResult<Widget>> FindForShopAsync(Guid shop_id, int skip, int take, string order_by = "", bool descending = false);
