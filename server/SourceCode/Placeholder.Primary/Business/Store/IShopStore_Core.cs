...
namespace Placeholder.Primary.Business.Store
{
    public partial interface IShopStore
    {
        ...
        Task<Shop> GetDocumentAsync(Guid shop_id);
        
        Task<bool> CreateDocumentAsync(Shop model);
        Task<bool> DeleteDocumentAsync(Shop model);
        
        [Obsolete("Use caution, this is expensive for multi-partition tables", false)]
        Task<ListResult<Shop>> FindAsync(int skip, int take, string order_by = "", bool descending = false, string keyword = "");
