...
namespace Stencil.Maui.Views.Standard.v1_0
{
    public partial class Carousel : ResourceDictionary, IDataViewComponent
    {
        ...
        public Task<IDataViewItemReference> PrepareBindingContextAsync(ICommandScope commandScope, IDataViewModel dataViewModel, IDataViewItem dataViewItem, DataTemplateSelector selector, string configuration_json)
        {
            return CoreUtility.ExecuteFunctionAsync<IDataViewItemReference>($"{COMPONENT_NAME}.{nameof(PrepareBindingContextAsync)}", async delegate ()
            {
                ...
                result.UIMinContentHeight = result.HeightRequest - 10; // don't set exact, pixel math
                ...
                result.Cells = new ObservableCollection<INestedDataViewModel>(cells);
        ...
        private void CarouselView_PositionChanged(object sender, PositionChangedEventArgs e)
        {
            ...
            {
                CarouselView view = (sender as CarouselView);
        ...
        {
            CoreUtility.ExecuteMethod("CarouselView_Loaded", delegate ()
            {
                CarouselView view = (sender as CarouselView);
                CarouselContext context = view?.BindingContext as CarouselContext;
                if(context != null)
        ...
        private void CarouselView_Unloaded(object sender, EventArgs e)
        {
            CoreUtility.ExecuteMethod("CarouselView_Unloaded", delegate ()
            {
                CarouselView view = (sender as CarouselView);
                CarouselContext context = view?.BindingContext as CarouselContext;
                if (context != null)
    ...
    public class CarouselContext : PreparedBindingContext, IStateEmitter
    {
        ...
        public bool UIAppliedFix { get; set; }
        ...
        [JsonIgnore]
        public double UIMinContentHeight { get; set; }

        [EditorBrowsable(EditorBrowsableState.Never)]
        [JsonIgnore]
        public CarouselView UIView { get; set; }
        ...
        [EditorBrowsable(EditorBrowsableState.Never)]
        [JsonIgnore]
        public ObservableCollection<INestedDataViewModel> Cells { get; set; }
                        ...
                        switch (state)
                        {
                            case "-1":
                                if(this.Position > 0)
                                {
                                    if(this.UIView != null)
                                    {
                                        this.UIView.ScrollTo(this.Position - 1, -1, ScrollToPosition.Center, true);
                                    }
                                    else
                                    {
                                ...
                                break;
                            case "1":
                                if (this.Position + 1 < this.Cells.Count)
                                {
                                    if (this.UIView != null)
                                    {
                                        this.UIView.ScrollTo(this.Position + 1, -1, ScrollToPosition.Center, true);
