...
{
    public class TrackedClass : TrackedClass<StencilAPI>
    {
        #region Constructor

        public TrackedClass(string trackPrefix)
    ...
    public class TrackedClass<TAPI> : PropertyClass
        where TAPI : StencilAPI
    {
        #region Constructor

        public TrackedClass(TAPI api, string trackPrefix)
        {
            this.API = api;
        ...
        /// </summary>
        [DebuggerNonUserCode]
        protected async Task ExecuteMethodOnMainThreadBegin(string name, Func<Task> action, Action<Exception> onError = null)
        {
            if (!MainThread.IsMainThread)
            {
                MainThread.BeginInvokeOnMainThread(async delegate ()
                {
                    await this.ExecuteMethodAsync(name, action, onError);
                });
            }
            else
            {
